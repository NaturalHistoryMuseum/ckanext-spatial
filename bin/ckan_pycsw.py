...
def setup_db(pycsw_config):
    ...
    database = pycsw_config.get(u'repository', u'database')
    table_name = pycsw_config.get(u'repository', u'table', u'records')

    ckan_columns = [
        Column(u'ckan_id', Text, index=True),
        Column(u'ckan_modified', Text),
    ]
...
def set_keywords(pycsw_config_file, pycsw_config, ckan_url, limit=20):
    ...
    log.info(u'Fetching tags from %s', ckan_url)
    url = ckan_url + u'api/tag_counts'
    response = requests.get(url)
    tags = response.json()

    log.info(u'Deriving top %d tags', limit)
    ...
    keywords = u','.join(u'%s' % tn[0] for tn in tags_sorted)

    log.info(u'Setting tags in pycsw configuration file %s', pycsw_config_file)
    pycsw_config.set(u'metadata:main', u'identification_keywords', keywords)
    with open(pycsw_config_file, u'wb') as configfile:
...
def load(pycsw_config, ckan_url):
    ...
    log.info(u'Started gathering CKAN datasets identifiers: {0}'.format(str(datetime.datetime.now())))

    query = u'api/search/dataset?qjson={"fl":"id,metadata_modified,extras_harvest_object_id,extras_metadata_source", "q":"harvest_object_id:[\\"\\" TO *]", "limit":1000, "start":%s}'
    ...
    while True:
        ...
        if not isinstance(listing, dict):
            raise RuntimeError, u'Wrong API response: %s' % listing
        results = listing.get(u'results')
        if not results:
            break
        for result in results:
            gathered_records[result[u'id']] = {
                u'metadata_modified': result[u'metadata_modified'],
                u'harvest_object_id': result[u'extras'][u'harvest_object_id'],
                u'source': result[u'extras'].get(u'metadata_source')
            }

        start = start + 1000
        log.debug(u'Gathered %s' % start)

    log.info(u'Gather finished ({0} datasets): {1}'.format(
    ...
    for key in set(gathered_records) & set(existing_records):
        if gathered_records[key][u'metadata_modified'] > existing_records[key]:
            changed.add(key)
    ...
    for ckan_id in new:
        ...
        if not record:
            log.info(u'Skipped record %s' % ckan_id)
            continue
        try:
            repo.insert(record, u'local', util.get_today_and_now())
            log.info(u'Inserted %s' % ckan_id)
        except Exception, err:
            log.error(u'ERROR: not inserted %s Error:%s' % (ckan_id, err))
    ...
    for ckan_id in changed:
        ...
        getattr(record, key)) \
        for key in record.__dict__.keys() if key != u'_sa_instance_state'])
        try:
            repo.session.begin()
            repo.session.query(repo.dataset).filter_by(
            ckan_id=ckan_id).update(update_dict)
            repo.session.commit()
            log.info(u'Changed %s' % ckan_id)
        except Exception, err:
            repo.session.rollback()
            raise RuntimeError, u'ERROR: %s' % str(err)
...
def get_record(context, repo, ckan_url, ckan_id, ckan_info):
    ...
    '''
    query = ckan_url + u'harvest/object/%s'
    url = query % ckan_info[u'harvest_object_id']
    response = requests.get(url)

    if ckan_info[u'source'] == u'arcgis':
        return

    try:
        xml = etree.parse(io.BytesIO(response.content))
    except Exception, err:
        log.error(u'Could not pass xml doc from %s, Error: %s' % (ckan_id, err))
        return

    try:
        record = metadata.parse_record(context, xml, repo)[0]
    except Exception, err:
        log.error(u'Could not extract metadata from %s, Error: %s' % (ckan_id, err))
        return

    if not record.identifier:
        record.identifier = ckan_id
    record.ckan_id = ckan_id
    record.ckan_modified = ckan_info[u'metadata_modified']
...
def _load_config(file_path):
    ...
    if not os.path.exists(abs_path):
        raise AssertionError(u'pycsw config file {0} does not exist.'.format(abs_path))
...
if __name__ == u'__main__':
    parser = argparse.ArgumentParser(
        description=u'\n'.split(usage)[0],
        usage=usage)
    parser.add_argument(u'command',
         help=u'Command to perform')

    parser.add_argument(u'-p', u'--pycsw_config',
         action=u'store', default=u'default.cfg',
         help=u'pycsw config file to use.')

    parser.add_argument(u'-u', u'--ckan_url',
         action=u'store',
    ...
    if arg.command == u'setup':
        setup_db(pycsw_config)
    elif arg.command in [u'load', u'set_keywords']:
        if not arg.ckan_url:
            raise AssertionError(u'You need to provide a CKAN URL with -u or --ckan_url')
        ckan_url = arg.ckan_url.rstrip('/') + '/'
        if arg.command == u'load':
            load(pycsw_config, ckan_url)
        else:
            set_keywords(arg.pycsw_config, pycsw_config, ckan_url)
    elif arg.command == u'clear':
        clear(pycsw_config)
    else:
        print u'Unknown command {0}'.format(arg.command)
        sys.exit(1)
