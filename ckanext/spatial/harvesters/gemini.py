...
debug_exception_mode = bool(os.getenv(u'DEBUG'))
...
class GeminiHarvester(SpatialHarvester):
    ...
    def import_stage(self, harvest_object):
        ...
        log.debug(u'Import stage for harvest object: %r', harvest_object)
        ...
        if harvest_object.content is None:
            self._save_object_error(u'Empty content for object %s' % harvest_object.id,
        ...
        try:
            ...
            log.error(u'Exception during import: %s' % text_traceback())
            if not str(e).strip():
                self._save_object_error(u'Error importing Gemini document.',
                                        harvest_object, u'Import')
            else:
                self._save_object_error(u'Error importing Gemini document: %s' % str(e),
                                        harvest_object, u'Import')
            raise
    ...
    def import_gemini_object(self, gemini_string):
        ...
        if not valid:
            ...
            log.error(u'Errors found for object with GUID %s:' % self.obj.guid)
            self._save_object_error(out, self.obj, u'Import')
    ...
    def write_package_from_gemini_string(self, content):
        ...
        gemini_guid = gemini_values[u'guid']

        # Save the metadata reference date in the Harvest Object
        try:
            metadata_modified_date = datetime.strptime(gemini_values[u'metadata-date'],
                                                       u'%Y-%m-%d')
        except ValueError:
            try:
                metadata_modified_date = datetime.strptime(
                    gemini_values[u'metadata-date'], u'%Y-%m-%dT%H:%M:%S')
            except:
                raise Exception(u'Could not extract reference date for GUID %s (%s)' % (
                    gemini_guid, gemini_values[u'metadata-date']))

        self.obj.metadata_modified_date = metadata_modified_date
        self.obj.save()

        last_harvested_object = Session.query(HarvestObject).filter(
            HarvestObject.guid == gemini_guid).filter(
            HarvestObject.current == True).all()

        if len(last_harvested_object) == 1:
            last_harvested_object = last_harvested_object[0]
        elif len(last_harvested_object) > 1:
            raise Exception(
                u'Application Error: more than one current record for GUID %s' %
                gemini_guid)
        ...
        if last_harvested_object:
            ...
            # needs to be updated
            if last_harvested_object.metadata_modified_date is None or \
                    last_harvested_object.metadata_modified_date < \
                    self.obj.metadata_modified_date or self.force_import or (
                    last_harvested_object.metadata_modified_date ==
                    self.obj.metadata_modified_date and
                    last_harvested_object.source.active is False):

                if self.force_import:
                    log.info(u'Import forced for object %s with GUID %s' % (
                        self.obj.id, gemini_guid))
                else:
                    log.info(
                        u'Package for object with GUID %s needs to be created or '
                        u'updated' % gemini_guid)

                package = last_harvested_object.package

                # If the package has a deleted state, we will only update it and
                # reactivate it if the
                # new document has a more recent modified date
                if package.state == u'deleted':
                    if last_harvested_object.metadata_modified_date < \
                            self.obj.metadata_modified_date:
                        log.info(
                            u'Package for object with GUID %s will be re-activated' %
                            gemini_guid)
                        reactivate_package = True
                    else:
                        log.info(
                            u'Remote record with GUID %s is not more recent than a '
                            u'deleted package, skipping... ' % gemini_guid)
                        return None
                ...
                if last_harvested_object.content != self.obj.content and \
                    ...
                    diff_generator = difflib.unified_diff(
                        last_harvested_object.content.split(u'\n'),
                        self.obj.content.split(u'\n'))
                    diff = u'\n'.join([line for line in diff_generator])
                    raise Exception(
                        u'The contents of document with GUID %s changed, but the '
                        u'metadata date has not been updated.\nDiff:\n%s' % (
                            gemini_guid, diff))
                else:
                    # The content hasn't changed, no need to update the package
                    log.info(
                        u'Document with GUID %s unchanged, skipping...' % (gemini_guid))
                return None
        else:
            log.info(
                u'No package with GEMINI guid %s found, let\'s create one' % gemini_guid)

        extras = {
            u'UKLP': u'True',
            u'harvest_object_id': self.obj.id
            }
        ...
        if len(gemini_values.get(u'progress', [])):
            extras[u'progress'] = gemini_values[u'progress'][0]
        else:
            extras[u'progress'] = u''

        extras[u'resource-type'] = gemini_values[u'resource-type'][0]
        ...
        extras[u'licence'] = gemini_values.get(u'use-constraints', u'')
        if len(extras[u'licence']):
            licence_url_extracted = self._extract_first_licence_url(extras[u'licence'])
            if licence_url_extracted:
                extras[u'licence_url'] = licence_url_extracted

        extras[u'access_constraints'] = gemini_values.get(
            u'limitations-on-public-access', u'')
        if gemini_values.has_key(u'temporal-extent-begin'):
            # gemini_values['temporal-extent-begin'].sort()
            extras[u'temporal_coverage-from'] = gemini_values[u'temporal-extent-begin']
        if gemini_values.has_key(u'temporal-extent-end'):
            # gemini_values['temporal-extent-end'].sort()
            extras[u'temporal_coverage-to'] = gemini_values[u'temporal-extent-end']

        # Save responsible organization roles
        provider, responsible_parties = self._process_responsible_organisation(
            gemini_values[u'responsible-organisation'])
        extras[u'provider'] = provider
        extras[u'responsible-party'] = u'; '.join(responsible_parties)

        if len(gemini_values[u'bbox']) > 0:
            extras[u'bbox-east-long'] = gemini_values[u'bbox'][0][u'east']
            extras[u'bbox-north-lat'] = gemini_values[u'bbox'][0][u'north']
            extras[u'bbox-south-lat'] = gemini_values[u'bbox'][0][u'south']
            extras[u'bbox-west-long'] = gemini_values[u'bbox'][0][u'west']

            # Construct a GeoJSON extent so ckanext-spatial can register the extent
            # geometry
            extent_string = self.extent_template.substitute(
                xmin=extras[u'bbox-east-long'],
                ymin=extras[u'bbox-south-lat'],
                xmax=extras[u'bbox-west-long'],
                ymax=extras[u'bbox-north-lat']
                )

            extras[u'spatial'] = extent_string.strip()

        tags = []
        for tag in gemini_values[u'tags']:
        ...
        package_dict = {
            u'title': gemini_values[u'title'],
            u'notes': gemini_values[u'abstract'],
            u'tags': tags,
            u'resources': []
            }

        if self.obj.source.publisher_id:
            package_dict[u'groups'] = [{
                u'id': self.obj.source.publisher_id
                }]

        if reactivate_package:
            package_dict[u'state'] = u'active'

        if package is None or package.title != gemini_values[u'title']:
            name = self.gen_new_name(gemini_values[u'title'])
            if not name:
        ...
        resource_locators = gemini_values.get(u'resource-locator', [])

        if len(resource_locators):
            for resource_locator in resource_locators:
                url = resource_locator.get(u'url', u'')
                if url:
                    resource_format = u''
                    resource = {}
                    if extras[u'resource-type'] == u'service':
                        ...
                        if self._is_wms(test_url):
                            ...
                            resource_format = u'WMS'
                    resource.update(
                        {
                            u'url': url,
                            u'name': resource_locator.get(u'name', u''),
                            u'description': resource_locator.get(
                                u'description') if resource_locator.get(
                                u'description') else u'Resource locator',
                            u'format': resource_format or None,
            ...
            verified_view_resources = [r for r in package_dict[u'resources'] if
                                       u'verified' in r and r[u'format'] == u'WMS']
            if len(verified_view_resources):
                verified_view_resources[0][u'ckan_recommended_wms_preview'] = True
            else:
                view_resources = [r for r in package_dict[u'resources'] if
                                  r[u'format'] == u'WMS']
                if len(view_resources):
                    view_resources[0][u'ckan_recommended_wms_preview'] = True
        ...
        if package == None:
            ...
            log.info(u'Created new package ID %s with GEMINI guid %s', package[u'id'],
                     gemini_guid)
        else:
            package = self._create_package_from_data(package_dict, package=package)
            log.info(u'Updated existing package ID %s with existing GEMINI guid %s',
                     package[u'id'], gemini_guid)

        # Flag the other objects of this source as not current anymore
        from ckanext.harvest.model import harvest_object_table
        u = update(harvest_object_table).where(
            harvest_object_table.c.package_id == bindparam(u'b_package_id')).values(
            current=False)
        Session.execute(u, params={
            u'b_package_id': package[u'id']
            })
        Session.commit()
        ...
        if not self.obj.package_id:
            self.obj.package_id = package[u'id']
    ...
    def _process_responsible_organisation(cls, responsible_organisations):
        ...
        for responsible_party in responsible_organisations:
            if responsible_party[u'role'] == u'owner':
                owners.append(responsible_party[u'organisation-name'])
            elif responsible_party[u'role'] == u'publisher':
                publishers.append(responsible_party[u'organisation-name'])

            if responsible_party[u'organisation-name'] in parties:
                if not responsible_party[u'role'] in parties[
                    responsible_party[u'organisation-name']]:
                    parties[responsible_party[u'organisation-name']].append(
                        responsible_party[u'role'])
            else:
                parties[responsible_party[u'organisation-name']] = [
                    responsible_party[u'role']]

        responsible_parties = []
        for party_name in parties:
            responsible_parties.append(
                u'%s (%s)' % (party_name, u', '.join(parties[party_name])))

        # Save provider in a separate extra:
        # first organization to have a role of 'owner', and if there is none,
        # first one with
        # a role of 'publisher'
        if len(owners):
            provider = owners[0]
    ...
    def gen_new_name(self, title):
        ...
        '''
        name = munge_title_to_name(title).replace(u'_', u'-')
        while u'--' in name:
    ...
    def _create_package_from_data(self, package_dict, package=None):
        ...
        if not package:
            ...
            action_function = toolkit.get_action(u'package_create')
        else:
            action_function = toolkit.get_action(u'package_update')
            package_dict[u'id'] = package.id

        try:
            package_dict = action_function(context, package_dict)
        except toolkit.ValidationError, e:
            raise Exception(u'Validation Error: %s' % str(e.error_summary))
            if debug_exception_mode:
                raise
    ...
    def get_gemini_string_and_guid(self, content, url=None):
        ...
        :param url: string giving info about the location of the XML to be
                    used only in validation errors (Default value = None)
        ...
        metadata_tag = u'{http://www.isotc211.org/2005/gmd}MD_Metadata'
        ...
        if gemini_xml is None:
            self._save_gather_error(
                u'Content is not a valid Gemini document without the gmd:MD_Metadata '
                u'element',
                self.harvest_job)
        ...
        try:
            gemini_guid = gemini_document.read_value(u'guid')
        except KeyError:
            gemini_guid = None
...
class GeminiCswHarvester(GeminiHarvester, SingletonPlugin):
    ...
    def gather_stage(self, harvest_job):
        ...
        log.debug(u'GeminiCswHarvester gather_stage for job: %r', harvest_job)
        # Get source URL
            ...
            for identifier in self.csw.getidentifiers(page=10):
                try:
                    ...
                    if identifier in used_identifiers:
                        log.error(
                            u'CSW identifier %r already used, skipping...' % identifier)
                        continue
    ...
    def fetch_stage(self, harvest_object):
        ...
        log.debug(u'GeminiCswHarvester fetch_stage for object: %r', harvest_object)
        ...
        try:
            ...
            harvest_object.content = record[u'xml']
            harvest_object.save()
        except Exception, e:
...
class GeminiDocHarvester(GeminiHarvester, SingletonPlugin):
    ...
    def info(self):
        ''' '''
        return {
            u'name': u'gemini-single',
            u'title': u'Single GEMINI 2 document',
            u'description': u'A single GEMINI 2.1 document'
            }
    ...
    def gather_stage(self, harvest_job):
        ...
        log.debug(u'GeminiDocHarvester gather_stage for job: %r', harvest_job)
        ...
        try:
            ...
            if gemini_guid:
                ...
                log.info(u'Got GUID %s' % gemini_guid)
                return [obj.id]
            else:
                self._save_gather_error(u'Could not get the GUID for source %s' % url,
                                        harvest_job)
                return None
        except Exception, e:
            self._save_gather_error(
                u'Error parsing the document. Is this a valid Gemini document?: %s ['
                u'%r]' % (
                    url, e), harvest_job)
            if debug_exception_mode:
                raise
...
class GeminiWafHarvester(GeminiHarvester, SingletonPlugin):
    ...
    def info(self):
        ...
        return {
            u'name': u'gemini-waf',
            u'title': u'Web Accessible Folder (WAF) - GEMINI',
            u'description': u'A Web Accessible Folder (WAF) displaying a list of '
                            u'GEMINI 2.1 documents'
            }
    ...
    def gather_stage(self, harvest_job):
        ...
        log.debug(u'GeminiWafHarvester gather_stage for job: %r', harvest_job)
        ...
        try:
            for url in self._extract_urls(content, url):
                try:
                    ...
                    msg = u'Couldn\'t harvest WAF link: %s: %s' % (url, e)
                    ...
                    try:
                        gemini_string, gemini_guid = self.get_gemini_string_and_guid(
                            content, url)
                        if gemini_guid:
                            log.debug(u'Got GUID %s' % gemini_guid)
                            # Create a new HarvestObject for this identifier
                            # Generally the content will be set in the fetch stage,
                            # but as we alredy
                            # have it, we might as well save a request
                            obj = HarvestObject(guid=gemini_guid,
                                                job=harvest_job,
                        ...
                        msg = u'Could not get GUID for source %s: %r' % (url, e)
                        self._save_gather_error(msg, harvest_job)
                        continue
        except Exception, e:
            msg = u'Error extracting URLs from %s' % url
            self._save_gather_error(msg, harvest_job)
            return None
        ...
        if len(ids) > 0:
            ...
            self._save_gather_error(u'Couldn\'t find any links to metadata files',
                                    harvest_job)
            return None
    ...
    def _extract_urls(self, content, base_url):
        ...
        try:
            ...
            msg = u'Couldn\'t parse content into a tree: %s: %s' % (inst, content)
            raise Exception(msg)
        urls = []
        for url in tree.xpath('//a/@href'):
            ...
            if not url:
                continue
            if u'?' in url:
                log.debug(u'Ignoring link in WAF because it has "?": %s', url)
                continue
            if '/' in url:
                log.debug(u'Ignoring link in WAF because it has "/": %s', url)
                continue
            if u'#' in url:
                ...
                continue
            if u'mailto:' in url:
                log.debug(u'Ignoring link in WAF because it has "mailto:": %s', url)
                continue
            log.debug(u'WAF contains file: %s', url)
            urls.append(url)
        ...
        base_url += '/'
        log.debug(u'WAF base URL: %s', base_url)
