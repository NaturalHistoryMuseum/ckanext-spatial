...
log = __import__(u'logging').getLogger(__name__)
...
class XsdValidator(BaseValidator):
    ...
    def _is_valid(cls, xml, xsd_filepath, xsd_name):
        ...
        :param xml: etree of the XML to be validated
        :param xsd_filepath: full path to the XSD file
        :param xsd_name: string describing the XSD
        :returns: (is_valid, [(error_message_string, error_line_number)])
        ...
        try:
            ...
            log.info(
                u'Validation errors found using schema {0}'.format(xsd_name))
            errors = []
...
class ISO19139EdenSchema(XsdValidator):
    ''' '''
    name = u'iso19139eden'
    title = u'ISO19139 XSD Schema (EDEN 2009-03-16)'
    ...
    def is_valid(cls, xml):
        ...
        '''
        xsd_path = u'xml/iso19139eden'

        metadata_type = cls.get_record_type(xml)

        if metadata_type in (u'dataset', u'series'):
            gmx_xsd_filepath = os.path.join(os.path.dirname(__file__),
                                            xsd_path, u'gmx/gmx.xsd')
            xsd_name = u'Dataset schema (gmx.xsd)'
            is_valid, errors = cls._is_valid(xml, gmx_xsd_filepath, xsd_name)
            if not is_valid:
                ...
                # compatibility
                errors.insert(
                    0, (u'{0} Validation Error'.format(xsd_name), None))
        elif metadata_type == u'service':
            gmx_and_srv_xsd_filepath = os.path.join(
                os.path.dirname(__file__), xsd_path, u'gmx_and_srv.xsd')
            xsd_name = u'Service schemas (gmx.xsd & srv.xsd)'
            ...
            is_valid = False
            errors = [(
                      u'Metadata type not recognised "%s" - cannot choose an ISO19139 '
                      u'validator.' %
                      metadata_type, None)]
        if is_valid:
            return True, []
...
class ISO19139NGDCSchema(XsdValidator):
    ...
    '''
    name = u'iso19139ngdc'
    title = u'ISO19139 XSD Schema (NGDC)'
    ...
    def is_valid(cls, xml):
        ...
        '''
        xsd_path = u'xml/iso19139ngdc'

        xsd_filepath = os.path.join(os.path.dirname(__file__),
                                    xsd_path, u'schema.xsd')
        return cls._is_valid(xml, xsd_filepath, u'NGDC Schema (schema.xsd)')
...
class FGDCSchema(XsdValidator):
    ...
    def is_valid(cls, xml):
        ...
        xsd_filepath = os.path.join(os.path.dirname(__file__),
                                    xsd_path, u'fgdc-std-001-1998.xsd')
        return cls._is_valid(
            xml, xsd_filepath, u'FGDC Schema (fgdc-std-001-1998.xsd)')
...
class SchematronValidator(BaseValidator):
    ...
    def is_valid(cls, xml):
        ...
        if not hasattr(cls, u'schematrons'):
            log.info(u'Compiling schematron "%s"', cls.title)
            cls.schematrons = cls.get_schematrons()
        for schematron in cls.schematrons:
            result = schematron(xml)
            errors = []
            for element in result.findall(
                    u'{http://purl.oclc.org/dsdl/svrl}failed-assert'):
    ...
    def extract_error_details(cls, failed_assert_element):
        ...
        :param failed_assert_element: 
        :returns: (error_message, fuller_error_details)

        '''
        assert_ = failed_assert_element.get(u'test')
        location = failed_assert_element.get(u'location')
        message_element = failed_assert_element.find(
            u'{http://purl.oclc.org/dsdl/svrl}text')
        message = message_element.text.strip()

        # TODO: Do we really need such detail on the error messages?
        return message, u'Error Message: "%s"  Error Location: "%s"  Error Assert: ' \
                        u'"%s"' % (
        message, location, assert_)
    ...
    def schematron(cls, schema):
        ...
        '''
        transforms = [
            u'xml/schematron/iso_dsdl_include.xsl',
            u'xml/schematron/iso_abstract_expand.xsl',
            u'xml/schematron/iso_svrl_for_xslt1.xsl',
            ]
...
class ConstraintsSchematron(SchematronValidator):
    ''' '''
    name = u'constraints'
    title = u'ISO19139 Table A.1 Constraints Schematron (Medin 1.3)'
    ...
    def get_schematrons(cls):
        ...
        with resource_stream(
                __name__,
                u'xml/medin/ISOTS19139A1Constraints_v1.3.sch') as schema:
            return [cls.schematron(schema)]


class ConstraintsSchematron14(SchematronValidator):
    ''' '''
    name = u'constraints-1.4'
    title = u'ISO19139 Table A.1 Constraints Schematron (Medin/Parslow 1.4)'
    ...
    def get_schematrons(cls):
        ...
        with resource_stream(
                __name__,
                u'xml/medin/ISOTS19139A1Constraints_v1.4.sch') as schema:
            return [cls.schematron(schema)]
...
class Gemini2Schematron(SchematronValidator):
    ''' '''
    name = u'gemini2'
    title = u'GEMINI 2.1 Schematron 1.2'
    ...
    def get_schematrons(cls):
        ...
        with resource_stream(
                __name__,
                u'xml/gemini2/gemini2-schematron-20110906-v1.2.sch') as schema:
            return [cls.schematron(schema)]
...
class Gemini2Schematron13(SchematronValidator):
    ''' '''
    name = u'gemini2-1.3'
    title = u'GEMINI 2.1 Schematron 1.3'

    @classmethod
    def get_schematrons(cls):
        ''' '''
        with resource_stream(__name__,
                             u'xml/gemini2/Gemini2_R1r3.sch') as schema:
            return [cls.schematron(schema)]
...
class Validators(object):
    ...
    def __init__(self, profiles=[u'iso19139', u'constraints', u'gemini2']):
...
def is_valid(self, xml):
    ...
    :param xml: etree of the XML to be validated
    :returns: (is_valid, failed_profile_name, [(error_message_string,
    error_line_number)])

    '''

    log.debug(u'Starting validation against profile(s) %s' % u','.join(self.profiles))
    for name in self.profiles:
        validator = self.validators[name]
        is_valid, error_message_list = validator.is_valid(xml)
        if not is_valid:
            # error_message_list.insert(0, 'Validating against "%s" profile
            # failed' % validator.title)
            log.info(u'Validating against "%s" profile failed' % validator.title)
            log.debug(u'%r', error_message_list)
            return False, validator.name, error_message_list
        log.debug(u'Validated against "%s"', validator.title)
    log.info(u'Validation passed')
    return True, None, []
...
if __name__ == u'__main__':
    ...
    if len(argv) == 3:
        profiles = argv[2].split(u',')
    else:
        profiles = [u'iso19139', u'constraints', u'gemini2']
